...
DallasTemperature::DallasTemperature(OneWire* _oneWire) // constructor of Dallas object, which require onewire object
  #if REQUIRESALARMS
...
{
  DeviceAddress deviceAddress; //typedef uint8_t DeviceAddress[8]; is a uint8_t array of 8 interger

  _wire->reset_search();
  devices = 0; // Reset the number of devices when we enumerate wire devices

  while (_wire->search(deviceAddress)) // function from onewire
  {
    if (validAddress(deviceAddress)) // function from ds header
    {
      if (!parasite && readPowerSupply(deviceAddress)) parasite = true; //READPOWERSUPPLY 0xB4  // Determine if device needs parasite power
		// readpowersupply // read device's power requirements
      ScratchPad scratchPad; // typedef uint8_t ScratchPad[9]; array of 9 integer

      readScratchPad(deviceAddress, scratchPad); // #define READSCRATCH     0xBE  // Read EEPROM
	
	  bitResolution = max(bitResolution, getResolution(deviceAddress));   // used to determine the delay amount needed to allow for the, temperature conversion to take place

      devices++;
    }
  }
  //the while loop will search and process each devices along the bus, if no more found, loop exits
  // if the device found and valid address: start processing
  // PROCESSING: double check parasite power supply
  // PROCESSING: pass scratchpad information from device to scratchpad
  // find the bitresolution????
  // done prcoessing and increase 1 device count after done.
}
